...
PROGRAM Init
  ...
  REAL, PARAMETER :: TSIM    = 1.0E+1*YEAR
  ...
  INTEGER, PARAMETER :: ONUM = 20          ! number of output time steps  !
...
!!$    CALL SetAttr(sources, "cooling", stemp)
    ...
    IF (CheckKey(physics,"problem",EULER3D_ROTSYMSGS)) &
...
!         "dynconst" / 0.1, &
         ...
         "dynconst" / 0.001, &
           ...
           "output/kinvis" / 1, &
           "output/dynvis" / 1)
IF (.NOT.(CheckKey(physics,"problem",EULER3D_ROTSYMSGS))) &
...
!             "logfile"  / logfile, &
  ...
  SUBROUTINE InitData(Mesh,Physics,Fluxes,Timedisc)
    ...
    ! get cartesian coordiantes for cell faces
    CALL Convert2Cartesian(Mesh%geometry,Mesh%fpos(:,:,1,:),wecart)
    CALL Convert2Cartesian(Mesh%geometry,Mesh%fpos(:,:,2,:),eacart)
    CALL Convert2Cartesian(Mesh%geometry,Mesh%fpos(:,:,3,:),socart)
    CALL Convert2Cartesian(Mesh%geometry,Mesh%fpos(:,:,4,:),nocart)
...
!!$             Timedisc%pvar(i,j,Physics%ZVELOCITY) = SQRT(MAX(0.0,-s*gs(s,z) / lambda))
       ...
       am = SUM(Mesh%bhz(Mesh%IMIN:Mesh%IMAX,Mesh%JMIN:Mesh%JMAX) &
