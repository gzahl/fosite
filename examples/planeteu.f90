...
!        "output/energy" / 1, &
  ...
  SUBROUTINE InitData(Timedisc,Mesh,Physics)
    ...
    REAL              :: cs
    REAL,DIMENSION(Mesh%IGMIN:Mesh%IGMAX,Mesh%JGMIN:Mesh%JGMAX,4,2) &
                      :: fcart
    REAL,DIMENSION(Mesh%IGMIN:Mesh%IGMAX,Mesh%JGMIN:Mesh%JGMAX) :: radius
    REAL,DIMENSION(Mesh%IGMIN:Mesh%IGMAX,Mesh%JGMIN:Mesh%JGMAX,4) :: fradius
    !------------------------------------------------------------------------!
    INTENT(IN)        :: Mesh
    ...
    IF(csiso.LE.0.) THEN
      CALL SetSoundSpeeds(Physics,Mesh,flaring * SQRT(GN*mcentral/radius))
      CALL SetSoundSpeeds(Physics,Mesh,flaring * SQRT(GN*mcentral/fradius))
