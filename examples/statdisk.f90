...
!              "vis"         / vis), &
  ...
  SUBROUTINE InitData(Timedisc,Mesh,Physics,Fluxes)
    ...
    REAL, DIMENSION(Mesh%IGMIN:Mesh%IGMAX,Mesh%JGMIN:Mesh%JGMAX,4,2) :: fcart
    !------------------------------------------------------------------------!
    INTENT(IN)        :: Mesh
    INTENT(INOUT)     :: Timedisc,Physics,Fluxes
    !------------------------------------------------------------------------!

    ! compute cartesian coordinates for face centered or corner positions
    SELECT CASE(GetType(Mesh))
    CASE(MIDPOINT)
      CALL Convert2Cartesian(Mesh%geometry,Mesh%fpos,fcart)
    CASE(TRAPEZOIDAL)
      CALL Convert2Cartesian(Mesh%geometry,Mesh%cpos,fcart)
    CASE DEFAULT
    ...
    DO i=Mesh%IGMIN,Mesh%IGMAX
      DO j=Mesh%JGMIN,Mesh%JGMAX
        bccsound(i,j) = CSISO*Mesh%bradius(i,j)**(0.5*C0)
        ! set y-velocity to local speed of sound for later use
        Timedisc%pvar(i,j,Physics%YVELOCITY) = bccsound(i,j)
        DO k=1,4
          r = SQRT(fcart(i,j,k,1)**2+fcart(i,j,k,2)**2)
          fcsound(i,j,k) = CSISO*r**(0.5*C0)
        END DO
      END DO
    END DO
    ...
    Timedisc%cvar(:,:,Physics%DENSITY) = Mesh%bradius**D0
