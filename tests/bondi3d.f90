...
!              "logfile"  / logfile, &
  ...
  SUBROUTINE InitData(Mesh,Physics,Fluxes,Timedisc)
    ...
    IF (GetType(Timedisc%Boundary(WEST)).EQ.FARFIELD) THEN
       DO j=Mesh%JGMIN,Mesh%JGMAX
          DO i=1,Mesh%GNUM
             ...
             Timedisc%Boundary(WEST)%data(i,j,Physics%XVELOCITY) = vr*Mesh%bposvec(Mesh%IMIN-i,j,1)/r
             Timedisc%Boundary(WEST)%data(i,j,Physics%YVELOCITY) = vr*Mesh%bposvec(Mesh%IMIN-i,j,2)/r
    ...
    IF (GetType(Timedisc%Boundary(EAST)).EQ.FARFIELD) THEN
       DO j=Mesh%JGMIN,Mesh%JGMAX
          DO i=1,Mesh%GNUM
             r = Mesh%bradius(Mesh%IMAX+i,j)
             CALL bondi(r/RB,GAMMA,RHOINF,CSINF,rho,vr)
             cs2 = CSINF**2 * (rho/RHOINF)**(GAMMA-1.0)
             Timedisc%Boundary(EAST)%data(i,j,Physics%DENSITY)   = rho
             Timedisc%Boundary(EAST)%data(i,j,Physics%XVELOCITY) = vr*Mesh%bposvec(Mesh%IMAX+i,j,1)/r
             Timedisc%Boundary(EAST)%data(i,j,Physics%YVELOCITY) = vr*Mesh%bposvec(Mesh%IMAX+i,j,2)/r
    ...
    IF (GetType(Timedisc%Boundary(SOUTH)).EQ.FARFIELD) THEN
       DO j=1,Mesh%GNUM
          DO i=Mesh%IGMIN,Mesh%IGMAX
             r = Mesh%bradius(i,Mesh%JMIN-j)
             CALL bondi(r/RB,GAMMA,RHOINF,CSINF,rho,vr)
             cs2 = CSINF**2 * (rho/RHOINF)**(GAMMA-1.0)
             Timedisc%Boundary(SOUTH)%data(i,j,Physics%DENSITY)   = rho
             Timedisc%Boundary(SOUTH)%data(i,j,Physics%XVELOCITY) = vr*Mesh%bposvec(i,Mesh%JMIN-j,1)/r
             Timedisc%Boundary(SOUTH)%data(i,j,Physics%YVELOCITY) = vr*Mesh%bposvec(i,Mesh%JMIN-j,2)/r
    ...
    IF (GetType(Timedisc%Boundary(NORTH)).EQ.FARFIELD) THEN
       DO j=1,Mesh%GNUM
          DO i=Mesh%IGMIN,Mesh%IGMAX
             r = Mesh%bradius(i,Mesh%JMAX+j)
             CALL bondi(r/RB,GAMMA,RHOINF,CSINF,rho,vr)
             cs2 = CSINF**2 * (rho/RHOINF)**(GAMMA-1.0)
             Timedisc%Boundary(NORTH)%data(i,j,Physics%DENSITY)   = rho
             Timedisc%Boundary(NORTH)%data(i,j,Physics%XVELOCITY) = vr*Mesh%bposvec(i,Mesh%JMAX+j,1)/r
             Timedisc%Boundary(NORTH)%data(i,j,Physics%YVELOCITY) = vr*Mesh%bposvec(i,Mesh%JMAX+j,2)/r
...
SUBROUTINE funcd(y,fy,dfy,plist)
  IMPLICIT NONE
  !------------------------------------------------------------------------!
  REAL, INTENT(IN)  :: y
