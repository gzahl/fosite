  ...
  SUBROUTINE MakeConfig(Sim, config)
    ...
    fluxes => Dict("order"     / LINEAR, &
         ! IMPORTANT: always use primitive reconstruction for NOH problem
             ...
             "limiter"   / PP, &         ! one of: minmod, monocent,...   !
...
!             "logfile"  / logfile, &
  ...
  SUBROUTINE InitData(Mesh,Physics,Timedisc)
    ...
    Timedisc%pvar(:,:,Physics%XVELOCITY) = VR0 * Mesh%bposvec(:,:,1) / Mesh%bradius(:,:)
    Timedisc%pvar(:,:,Physics%YVELOCITY) = VR0 * Mesh%bposvec(:,:,2) / Mesh%bradius(:,:)
